...
import { createUserWithEmailAndPassword, updateProfile, getIdToken, reload, signInWithPopup } from "firebase/auth";
...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (displayName) {
        ...
        // Asegurar que el displayName esté disponible en el cliente inmediatamente
      ...
      await fetch('/api/users/ensure', {
        ...
        body: JSON.stringify({ email: cred.user.email, displayName: displayName || cred.user.displayName }),
      }).catch(() => {});
  ...
  async function handleGoogleSignUp() {
    try {
      ...
      const msg = err instanceof Error ? err.message : "No se pudo crear la cuenta con Google";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-teal-50 pt-28 pb-10 px-4">
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg p-6">
        ...
        <p className="text-sm text-gray-600 mb-6 text-center">Únete a FocusMeet</p>
        ...
        <p className="text-sm text-gray-600 mt-4 text-center">
          ¿Ya tienes cuenta? {" "}
          <Link href="/login" className="text-indigo-600 hover:underline font-medium">Iniciar sesión</Link>
        </p>
      </div>
    </div>
  );
